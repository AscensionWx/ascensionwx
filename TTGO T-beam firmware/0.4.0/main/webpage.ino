...
String html PROGMEM = R"(
  ...
  //String("<p>Firmware: "+APP_VERSION+"</p>") + // needs fixing
          ...
          if ( currentLine.length() == 0 ) // two newline characters in a row... end of client HTTP request
          {
            ...
            if ( index >= 0 && _first_page_visited_time!=0 && time_loop_check( _first_page_visited_time, 3000 ) )
            {
              ...
              if ( deveui.length() == 16 && appkey.length() == 32 )
              {
                ...
                Serial.println("User inputs passed. Keys saved and device intentionally rebooting.");
              
                // Construct new html page
                newpage = webpage + "<p>Keys saved. Device rebooting.</p>";
                
              } 
              else // user input had an error
              {

                Serial.println("User's input had a key length error. Loading new webpage.");
