...
bool wait_for_gps_time_and_location()
{
  if( !if_gps_on() ) {
  ...
  Serial.println("Waiting a bit for GPS data to populate over Serial");
  uint64_t begin_gps_wait = millis();
  while( !time_loop_check(begin_gps_wait, 10*1000) ) // Wait maximum of 10 seconds
  {
    // Needs to be repeatedly called to fill the _gps object time/lat/lon buffers
    gps_read();

    // GPS time age is <1.5 seconds and lat/lon age is less than int32 max (its own internal default)
    if ( _gps.time.age() < 1500 && _gps.location.age() < 4200000000 )
    { 
      return true;
    }
  }
