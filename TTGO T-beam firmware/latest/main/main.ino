...
const char * APP_VERSION = "0.4.4";
String APP_VERSION_STR = "0.4.4";
...
const int gps_send_interval = hr_milliseconds*22*3; // roughly 3 days
...
void loop() {
  ...
  // Sleep until RX1 window opens
  Serial.println("Observation sent. Waiting for RX windows.");

  // Want to sleep here, but apparently will not work with LMIC, because
  //   arduino-lmic does its own time calculations until RX1 and RX2
  //sleep_light_millis(800);

  // Stay awake exactly until RX windows close after observation
  wait_for_message_txcomplete();
  Serial.println("TXComplete received.");
  
  /////////////////  GPS Send Message and Time Refresh  ///////////////
  // Every 3 days, we turn on GPS for 5 minutes and send GPS message //
  /////////////////////////////////////////////////////////////////////
  if( time_loop_check( last_gps_send , gps_send_interval ) )
  {
    ...
    // Have GPS on for 5 minutes, while ESP32 is asleep
    ...
    // Send GPS mesage
    send_gps();
    time_loop_wait( 3000 ); // Wait for lmic and receive pending downlinks
