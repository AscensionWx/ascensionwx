...
String html PROGMEM = R"(
  ...
  <title>dClimate Weather Station</title>
  ...
  <body>
  <h1>dClimate Weather Station</h1>)" + 
        ...
        if ( c== '\n' )
        {
          if ( currentLine.length() == 0 ) // two newline characters in a row... end of client HTTP request
          {
            ...
            if ( index >= 0) // 
            {
              ...
              screen_print(" \n \n"); // one line
              screen_print("Listening for response...\n \n");
              
              bool status_sent = false;
              while ( millis() - auth_send_time < 15000 && !if_received_downlink()) // wait for nodered to query blockchain
              {
                os_runloop_once();

                // The downlink will happen after the next message, so
                //   we send an additional status message to receive downlinked field
                if ( millis() - auth_send_time > 8000 && !status_sent )
                {
                  send_status();
                  status_sent = true;
                }
